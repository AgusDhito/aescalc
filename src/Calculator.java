	...
	private void initialize() {
		...
		btnOpenDEncrypt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (result == JFileChooser.APPROVE_OPTION) {
				    File pathDEncrypt = fs.getSelectedFile();
				    textFileDEncrypt.setText(pathDEncrypt.getAbsolutePath());
				}
			}
		});
		...
		btnOpenKey.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int result = fs.showOpenDialog(null);
				if (result == JFileChooser.APPROVE_OPTION) {
				    File pathKey = fs.getSelectedFile();
				    textFileKey.setText(pathKey.getAbsolutePath());
				}
			}
		});
		...
		btnEncrypt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				textResult.setText(AESBackEnd.start(textFileDEncrypt.getText(), textFileKey.getText(), "ENCRYPT"));
		...
		btnDecrypt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String keyLength = getSelectedButton(keyGroup);
				String[] length = keyLength.split(" ");
				textResult.setText(AESBackEnd.start(textFileDEncrypt.getText(), textFileKey.getText(), "DECRYPT"));
				// gunakan length[0] untuk mendapatkan panjang key
